...
class MixVisionTransformer(nn.Module):
    def __init__(
        ...
        # self.classifier = nn.Linear(embed_dims[3], num_classes)
        self.classifiers = nn.ModuleList([nn.Linear(embed_dims[i], num_classes) for i in range(len(embed_dims))])
        self.apply(self._init_weights)
    ...
    def forward(self, x):
        ...
        for i, feat in enumerate(features):
            # 池化每个阶段的特征图
            pooled_feat = F.adaptive_avg_pool2d(feat, (1, 1)).squeeze(2).squeeze(2)
            
            # 输入到相应的分类器进行分类
            logit = self.classifiers[i](pooled_feat)
            logits.append(logit)
        ...
        # _, predicted_classes = torch.max(x, dim=1)  # Get the predicted classes
