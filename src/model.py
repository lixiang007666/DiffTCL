...
class DecoderDenoisingModel(pl.LightningModule):
    def __init__(
        ...
        self.net = smp.create_model(
            ...
            # encoder_weights="imagenet" if mode == "decoder" else None, # imagenet
        )

        # Note: for self pretrain, not use create_model's encoder_weights para
        # if mode == "decoder":
        #     state_dict = torch.load("pretrain_trans_gamma.pth")
        #     del state_dict['classifiers.0.weight']
        #     del state_dict['classifiers.0.bias']
        #     del state_dict['classifiers.1.weight']
        #     del state_dict['classifiers.1.bias']
        #     del state_dict['classifiers.2.weight']
        #     del state_dict['classifiers.2.bias']
        #     del state_dict['classifiers.3.weight']
        #     del state_dict['classifiers.3.bias']
        #     # state_dict['patch_embed1.proj.weight'] = state_dict['patch_embed1.proj.weight'].repeat(1, 3, 1, 1)
    ...
    def denoise_step(self, x, mode="train"):
        ...
        # print(noise.shape)
        # noise = F.interpolate(noise, size=(2048, 2048), mode='nearest')
