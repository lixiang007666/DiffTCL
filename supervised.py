...
def evaluate(model, loader, mode, cfg):
    ...
    with torch.no_grad():
        for img, mask, id in loader:
            ...
            pred = F.interpolate(pred.unsqueeze(0).float(), size=target_shape, mode='nearest').squeeze(0)
...
def evaluate_dice(model, loader, mode, cfg):
    model.eval()
    assert mode in ['original', 'center_crop', 'sliding_window']
    ...
    with torch.no_grad():
        for img, mask, id in loader:
            ...
            for c in range(cfg['nclass']):
                ...
                true_class = true_mask == c
                intersection = np.logical_and(pred_class, true_class).sum()
                dice = 2.0 * intersection / (pred_class.sum() + true_class.sum() + 1e-10)
                dice_scores.append(dice)

            dice_meter.update(np.array(dice_scores))

    mean_dice = dice_meter.avg * 100.0

    return mean_dice.sum()/cfg['nclass'], dice_meter.avg * 100.0
